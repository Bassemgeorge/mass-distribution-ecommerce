...
const emptyProduct: Omit<Product, "id"> = {
  ...
  price_per_piece: 0, price_per_carton: 0, case_count: 1,
  has_tax: false, image: null, is_active: true,
};
...
export default function AdminProductsPage() {
  ...
  const filtered = products.filter((p) => {
    ...
    return !q || p.name_en.toLowerCase().includes(q) || (p.brand ?? "").toLowerCase().includes(q) || (p.category ?? "").toLowerCase().includes(q);
  });
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-400 text-sm">{products.length} total products in Supabase</p>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="text-right px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide hidden lg:table-cell">Carton</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map((p) => (
                  <tr key={p.id} className={`hover:bg-gray-50 transition-colors ${p.is_active === false ? "opacity-50" : ""}`}>
                    ...
                    <td className="px-4 py-3 text-right text-sm font-semibold text-[#111111]">EGP {(p.price_per_piece ?? 0).toFixed(2)}</td>
                    <td className="px-4 py-3 text-right text-xs text-gray-400 hidden lg:table-cell">EGP {(p.price_per_carton ?? 0).toFixed(2)}</td>
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={() => setEdit({ ...p })}
                        className="text-gray-400 hover:text-[#1B4D2E] transition-colors flex items-center gap-1 text-xs"
                      >
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="number" step="0.01" value={editItem.price_per_piece} onChange={(e) => setEdit({ ...editItem, price_per_piece: parseFloat(e.target.value) || 0 })} className={inp} />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1.5">Carton Price (EGP)</label>
                <input type="number" step="0.01" value={editItem.price_per_carton} onChange={(e) => setEdit({ ...editItem, price_per_carton: parseFloat(e.target.value) || 0 })} className={inp} />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1.5">Case Count (units per carton)</label>
                <input type="number" value={editItem.case_count} onChange={(e) => setEdit({ ...editItem, case_count: parseInt(e.target.value) || 1 })} className={inp} />
              </div>
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={editItem.is_active !== false} onChange={(e) => setEdit({ ...editItem, is_active: e.target.checked })} className="accent-[#1B4D2E] w-4 h-4" />
                <span className="text-sm text-gray-600">Product is active (visible on store)</span>
              </label>
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={() => setEdit(null)} className="flex-1 border border-gray-200 text-gray-600 py-2.5 rounded-lg text-sm hover:bg-gray-50 transition-colors">Cancel</button>
              <button onClick={saveEdit} disabled={saving} className="flex-1 bg-[#1B4D2E] text-white py-2.5 rounded-lg text-sm font-semibold hover:bg-[#163d24] transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => { setAdd(false); setNewProd({ ...emptyProduct }); }} className="text-gray-400 hover:text-gray-600"><X size={18} /></button>
            </div>
            <div className="space-y-3">
              {[
                { label: "English Name *", field: "name_en", type: "text" },
                { label: "Arabic Name",    field: "name_ar", type: "text" },
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input type="number" step="0.01" value={newProd.price_per_piece} onChange={(e) => setNewProd((p) => ({ ...p, price_per_piece: parseFloat(e.target.value) || 0 }))} className={inp} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1.5">Carton Price</label>
                  <input type="number" step="0.01" value={newProd.price_per_carton} onChange={(e) => setNewProd((p) => ({ ...p, price_per_carton: parseFloat(e.target.value) || 0 }))} className={inp} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1.5">Case Count</label>
                  <input type="number" value={newProd.case_count} onChange={(e) => setNewProd((p) => ({ ...p, case_count: parseInt(e.target.value) || 1 }))} className={inp} />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1.5">Image URL (optional)</label>
                <input type="text" value={newProd.image ?? ""} onChange={(e) => setNewProd((p) => ({ ...p, image: e.target.value || null }))} className={inp} placeholder="https://…" />
              </div>
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer text-sm">
                  <input type="checkbox" checked={newProd.has_tax} onChange={(e) => setNewProd((p) => ({ ...p, has_tax: e.target.checked }))} className="accent-[#1B4D2E]" />
                  Has Tax (14%)
                </label>
                <label className="flex items-center gap-2 cursor-pointer text-sm">
                  <input type="checkbox" checked={newProd.is_active !== false} onChange={(e) => setNewProd((p) => ({ ...p, is_active: e.target.checked }))} className="accent-[#1B4D2E]" />
                  Active
                </label>
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={() => { setAdd(false); setNewProd({ ...emptyProduct }); }} className="flex-1 border border-gray-200 text-gray-600 py-2.5 rounded-lg text-sm hover:bg-gray-50 transition-colors">Cancel</button>
              <button onClick={addProduct} disabled={saving || !newProd.name_en || !newProd.brand} className="flex-1 bg-[#1B4D2E] text-white py-2.5 rounded-lg text-sm font-semibold hover:bg-[#163d24] transition-colors disabled:opacity-60 flex items-center justify-center gap-2">

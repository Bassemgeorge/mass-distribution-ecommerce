        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {items.map(({ product, quantity }) => (
              <div key={product.id} className="flex items-center gap-4 bg-white border border-gray-200 rounded-xl p-4 hover:border-gray-300 transition-colors">
                ...
                <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden flex-shrink-0">
                  <button onClick={() => quantity > 1 ? update(product.id, quantity - 1) : remove(product.id)} className="px-2.5 py-1.5 text-sm font-semibold hover:bg-gray-50 transition-colors">−</button>
                  <span className="px-2.5 py-1.5 text-sm font-semibold border-x border-gray-200 min-w-[2rem] text-center">{quantity}</span>
                  <button onClick={() => update(product.id, quantity + 1)} className="px-2.5 py-1.5 text-sm font-semibold hover:bg-gray-50 transition-colors">+</button>
                </div>
          ...
          <div>
            <div className="bg-[#F7F7F5] rounded-xl p-5 border border-gray-200 sticky top-24">
              ...
              <Link href="/checkout" className="block w-full text-center bg-[#1B4D2E] text-white font-semibold py-3 rounded-lg hover:bg-[#163d24] transition-colors text-sm">
                Proceed to Checkout
              </Link>
